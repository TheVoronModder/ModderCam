...
# Configure /etc/default/moddercam from flags
if [ "$DEVICE" = "auto" ]; then
...
if [ "$AUTOSTART" = "1" ]; then
  ...
  echo "[ModderCam] You can now start the service: sudo systemctl start moddercam"
fi

# Restart Klipper so macros are available
